...
def get_inventory_item_id(variant):
    return variant.get("inventoryItem", {}).get("id")


def get_hsn_code(variant):
    return variant.get("inventoryItem", {}).get("harmonizedSystemCode", "00000000")
...
def generate_gst_invoice_data(shopify_order, seller_details):
    ...
    for idx, item in enumerate(shopify_order["line_items"]):
        ...
        inventory_item_id = get_inventory_item_id(variant)
        hsn_code = get_hsn_code(variant)
        ...
        invoice_data["ItemList"].append(
            {
                ...
                "IsServc": "N",
                "HsnCd": hsn_code,
                "Barcde": "",  # Assuming barcode is not directly available from GraphQL response
                "Qty": quantity,
