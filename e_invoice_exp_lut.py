...
def get_shopify_order(order_id):
    ...
    if response_data and "data" in response_data and response_data["data"]["order"]:
        ...
        raise Exception(
            f"Failed to fetch order details for order ID: {order_id} using GraphQL. Response: {response_data}"
        )
...
def generate_gst_invoice_data(shopify_order, seller_details):
    ...
    for idx, item_node in enumerate(shopify_order["lineItems"]["nodes"]):
        item = item_node
        hsn_code = (
            item.get("variant", {})
            .get("inventoryItem", {})
