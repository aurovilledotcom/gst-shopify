...
def get_shopify_order_graphql(order_id):
    query = f"""
    query {{
...
def generate_gst_invoice_data(shopify_order, seller_details):
    ...
    for idx, line_item in enumerate(shopify_order["lineItems"]["edges"]):
        item = line_item["node"]
        ...
        invoice_data["ItemList"].append(
            {
                ...
                "IsServc": "N",
                "HsnCd": variant.get("inventoryItem", {}).get(
...
def create_e_invoice_lut(order_id):
    ...
    shopify_order = get_shopify_order_graphql(order_id)
