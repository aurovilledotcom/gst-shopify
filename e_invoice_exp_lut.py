...
def get_shopify_order(order_id):
    ...
    if "data" not in response_data or "order" not in response_data["data"]:
...
def generate_gst_invoice_data(shopify_order, seller_details):
    ...
    invoice_data = {
        ...
        "BuyerDtls": {
            "Gstin": "URP",
            "LglNm": f"{shopify_order.get('customer', {}).get('firstName', '')} "
            f"{shopify_order.get('customer', {}).get('lastName', '')}",
            "Pos": "96",
    ...
    for idx, item_edge in enumerate(
        shopify_order.get("lineItems", {}).get("edges", [])
    ):
        item = item_edge["node"]
        hsn_code = (
            item.get("variant", {})
